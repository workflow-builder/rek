...
def install_tools():
    ...
    if not command_exists("findomain"):
        ...
        sys.exit(1)
        # print(colored("[*] Downloading findomain...", "yellow"))
        # url = "https://github.com/findomain/findomain/releases/latest/download/findomain-windows.exe"
        # dest = TOOLS_DIR / "findomain.exe"
        # urllib.request.urlretrieve(url, dest)
        # print(colored(f"[+] findomain saved to {dest}", "green"))
...
def check_prerequisites():
    ...
    gf_dir = Path.home() / ".gf"
    if gf_dir.exists():
        print(f"{'gf patterns':<20}" + colored("Available", "green"))
    else:
        print(f"{'gf patterns':<20}" + colored("Missing", "red"))
        all_ok = False
...
def load_config():
    ...
    if CONFIG_FILE.exists():
        ...
        t = input(colored("[?] Default number of threads (default: 250): ", "yellow")).strip()
        if t:
            try: THREADS = int(t)
            except ValueError: pass
...
def subdomain_enumeration():
    ...
    run(["subfinder", "-d", TARGET_DOMAIN, "-all", "-recursive", "-silent",
         "-o", str(subs_dir / "subPrecursive.txt")], cwd=str(subs_dir))
    ...
    if GITHUB_API_TOKEN:
        ...
        run(["github-subdomains", "-d", TARGET_DOMAIN, "-t", GITHUB_API_TOKEN,
             "-o", str(subs_dir / "github-subs.txt")], cwd=str(subs_dir))
    else:
    ...
    if GITLAB_API_TOKEN:
        ...
        run(["gitlab-subdomains", "-d", TARGET_DOMAIN, "-t", GITLAB_API_TOKEN,
             "-o", str(subs_dir / "gitlab-subs.txt")], cwd=str(subs_dir))
    else:
...
def refresh_resolvers():
    print(colored("[*] Refreshing DNS resolvers...", "yellow"))
    urllib.request.urlretrieve(
        "https://raw.githubusercontent.com/blechschmidt/massdns/master/lists/resolvers.txt",
        RESOLVERS_FILE
    )
    if RESOLVERS_FILE.exists() and RESOLVERS_FILE.stat().st_size > 0:
        print(colored("[+] Resolvers refreshed", "green"))
    else:
        print(colored("[!] Failed to refresh resolvers", "red"))
...
def subdomain_permutation():
    ...
    print(colored("[*] Refreshing resolvers...", "yellow"))
    refresh_resolvers()
    ...
    if command_exists("puredns"):
        ...
        p3 = subprocess.Popen(
            ...
            stdout=subprocess.PIPE, stdin=subprocess.PIPE, stderr=subprocess.DEVNULL, env=merged_env4
        )
    ...
    write_lines(subs_dir / "sorted-subs.txt", sorted(final))
...
def identify_live_subdomains():
    ...
    run(["httpx",
         "-l", str(subs_dir / "sorted-subs.txt"),
         "-threads", str(THREADS),
...
def content_discovery():
    ...
    p = subprocess.Popen(
        ["gau", "--threads", "50", "--blacklist", "jpg,jpeg,png,gif,svg,css,tff,woff,woff2,ico,tif,tiff,webp"],
        stdin=subprocess.PIPE, stdout=subprocess.PIPE,
        stderr=subprocess.DEVNULL, env=merged_env
    )
...
def analyze_vulnerabilities():
    ...
    for vuln in vuln_types:
        if not (gf_dir / f"{vuln}.json").exists():
            print(colored(f"[!] GF pattern for {vuln} not found, skipping...", "yellow"))
            continue
        p = subprocess.Popen(
            ["gf", vuln],
            stdin=subprocess.PIPE, stdout=subprocess.PIPE,
...
def categorize_endpoints():
    ...
    for category, patterns in categories.items():
        ...
        write_lines(endpoints_dir / f"{category}-endpd", matches)
...
def generate_report():
    ...
    add_count(RESULTS_DIR / "subdomains" / "sorted-subs.txt", "Total unique subdomains discovered")
    ...
    for ext in ["json", "bak", "pdf"]:
        ef = RESULTS_DIR / "endpoints" / f"{ext}-endpd"
